...
std::map<std::string, std::string>
  parseYAML(const std::string& filePath)
{
...
{
  ConfigSetting cfg;
  auto config = parseYAML(config_fn);
  // pre-preocess
  cfg.ds_size_                      = std::stod(config["ds_size"]);
  cfg.maximum_corner_num_           = std::stoi(config["maximum_corner_num"]);
  // key points
  ...
  cfg.proj_dis_min_                 = std::stoi(config["proj_dis_min"]);
  cfg.proj_dis_max_                 = std::stoi(config["proj_dis_max"]);
  cfg.corner_thre_                  = std::stoi(config["corner_thre"]);
  // std descriptor
  cfg.descriptor_near_num_          = std::stoi(config["descriptor_near_num"]);
  cfg.descriptor_min_len_           = std::stod(config["descriptor_min_len"]);
  cfg.descriptor_max_len_           = std::stod(config["descriptor_max_len"]);
  cfg.non_max_suppression_radius_   = std::stod(config["non_max_suppression_radius"]);
  cfg.std_side_resolution_          = std::stod(config["std_side_resolution"]);
  // candidate search
  cfg.skip_near_num_                = std::stoi(config["skip_near_num"]);
  cfg.candidate_num_                = std::stoi(config["candidate_num"]);
  cfg.sub_frame_num_                = std::stoi(config["sub_frame_num"]);
  cfg.vertex_diff_threshold_        = std::stod(config["vertex_diff_threshold"]);
  cfg.rough_dis_threshold_          = std::stod(config["rough_dis_threshold"]);
  cfg.normal_threshold_             = std::stod(config["normal_threshold"]);
  cfg.dis_threshold_                = std::stod(config["dis_threshold"]);
  cfg.icp_threshold_                = std::stod(config["icp_threshold"]);
  /* for pseudo loop gt */
  cfg.result_dir                    = config["result_dir"];
  cfg.seq_name                      = config["seq_name"];
  cfg.align                         = (config["align"] == "true" || config["align"] == "True") ? true : false;
  cfg.z_max                         = std::stod(config["z_max"]);
  cfg.exid                          = config["exid"];
  cfg.is_benchmark                  = (config["is_benchmark"] == "true" || config["is_benchmark"] == "True") ? true : false;
  return cfg;
}
